# Thread UI Implementation Prompts

## 1. Simple List View

```
Implement a clean, minimal thread list view component that displays all threads in a tabular format. The table should have the following columns:
...
```
Create a simple filter control consisting of four buttons positioned above the thread list:
...
```
Develop a thread detail page that shows comprehensive information about a specific thread. The page should include:
...
```
Create a simple application header that displays a summary of thread counts by status. The header should:
...
```
Implement a copy-to-clipboard feature for thread IDs and output text. Add small copy icons next to:
